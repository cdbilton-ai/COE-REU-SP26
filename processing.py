...
# ================================================================
print("="*60)
print("  ENSIGHT TO VTK + MULTI-VARIABLE SCREENSHOTS")
print("="*60)

raw_path = input("Drag and drop your .encas file here > ")
...
# ================================================================
try:
    print("\n1. Loading EnSight data...")
    reader = OpenDataFile(input_file)
    reader.UpdatePipeline()

    print("2. Merging Multi-Block data...")
    merged_data = MergeBlocks(Input=reader)
    merged_data.UpdatePipeline()

    print(f"3. Saving VTK file: {os.path.basename(output_vtk)}...")
    SaveData(output_vtk, proxy=merged_data)

except Exception as e:
    print(f"\n[CRITICAL ERROR] Failed during load/convert. Details: {e}")
    sys.exit(1)
...
HideAll(renderView)
display = Show(merged_data, renderView)
